...
@Transactional
public class WeixinAccountTokenTask implements Job{
	...
	public void run() {
		...
		try {	
			...
			String hql = "from WeixinAccountEntity where addtoekntime < to_date('"+currentDatetime+"','yyyy-MM-dd HH24:mi:ss')";
			List<WeixinAccountEntity> list = systemService.findHql(hql);
			
			org.jeecgframework.core.util.LogUtil.info("===================定时任务【重置超过2小时失效token】开始"+list.size()+"===================");
		...
		org.jeecgframework.core.util.LogUtil.info("===================定时任务【重置超过2小时失效token】===================");
		long end = System.currentTimeMillis();
		long times = end - start;
