package com.rpp.rate.service.impl;
...
@Service("rppRateService")
@Transactional
public class RppRateServiceImpl extends CommonServiceImpl implements RppRateServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(RppRateEntity t) throws Exception{
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(RppRateEntity t) throws Exception{
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(RppRateEntity t) throws Exception{
	 	//-----------------sql增强 end------------------------------
	 	
	 	//-----------------java增强 start---------------------------
	}
 	
 	private Map<String,Object> populationMap(RppRateEntity t){
		...
		map.put("default_value", t.getDefaultValue());
		map.put("min_value", t.getMinValue());
		map.put("max_value", t.getMaxValue());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,RppRateEntity t){
 		...
 		sql  = sql.replace("#{default_value}",String.valueOf(t.getDefaultValue()));
 		sql  = sql.replace("#{min_value}",String.valueOf(t.getMinValue()));
 		sql  = sql.replace("#{max_value}",String.valueOf(t.getMaxValue()));
	 ...
	 */
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("rpp_rate",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
 	...
 	private void executeSqlEnhance(String sqlEnhance,RppRateEntity t){
