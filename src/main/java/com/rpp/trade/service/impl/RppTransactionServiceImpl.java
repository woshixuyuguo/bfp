package com.rpp.trade.service.impl;
...
@Service("rppTransactionService")
@Transactional
public class RppTransactionServiceImpl extends CommonServiceImpl implements RppTransactionServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(RppTransactionEntity t) throws Exception{
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(RppTransactionEntity t) throws Exception{
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(RppTransactionEntity t) throws Exception{
	 	//-----------------sql增强 end------------------------------
	 	
	 	//-----------------java增强 start---------------------------
	}
 	
 	private Map<String,Object> populationMap(RppTransactionEntity t){
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,RppTransactionEntity t){
 		...
 		sql  = sql.replace("#{merc_code}",String.valueOf(t.getMercCode()));
 		sql  = sql.replace("#{org_code}",String.valueOf(t.getOrgCode()));
 		sql  = sql.replace("#{rate_code}",String.valueOf(t.getRateCode()));
 		sql  = sql.replace("#{rate}",String.valueOf(t.getRate()));
 		sql  = sql.replace("#{org_rate}",String.valueOf(t.getOrgRate()));
 		sql  = sql.replace("#{money}",String.valueOf(t.getMoney()));
 		sql  = sql.replace("#{money_fee}",String.valueOf(t.getMoneyFee()));
 		sql  = sql.replace("#{money_act}",String.valueOf(t.getMoneyAct()));
 		sql  = sql.replace("#{state}",String.valueOf(t.getState()));
 		sql  = sql.replace("#{enter}",String.valueOf(t.getEnter()));
 		sql  = sql.replace("#{out_order_no}",String.valueOf(t.getOutOrderNo()));
 		...
 		sql  = sql.replace("#{sysn_notify_url}",String.valueOf(t.getSysnNotifyUrl()));
	 ...
	 */
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("rpp_transaction",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
 	...
 	private void executeSqlEnhance(String sqlEnhance,RppTransactionEntity t){
