...
import com.rpp.merc.entity.TSMerchant;
...
//@Scope("prototype")
@Controller
@RequestMapping("/agtController")
public class AgtController extends BaseController {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(AgtController.class);
	...
	public String changepassword(HttpServletRequest request) {
		TSUser user = ResourceUtil.getSessionUser();
		request.setAttribute("user", user);
		return "com/rpp/agent/changepassword";
	}
	...
	public String home(HttpServletRequest request) {
		TSUser user = ResourceUtil.getSessionUser();
		TSAgent tsAgent=systemService.findUniqueByProperty(TSAgent.class, "id", user.getId());
		request.setAttribute("user", tsAgent);
		return "com/rpp/agent/home";
	}
	...
	public String changeportrait(HttpServletRequest request) {
		TSUser user = ResourceUtil.getSessionUser();
		request.setAttribute("user", user);
		return "com/rpp/agent/changeportrait";
	}
	/**
	 * 修改密码
	 *
	...
	public ModelAndView changepasswordforuser(TSUser user, HttpServletRequest req) {
		...
		return new ModelAndView("com/rpp/agent/adminchangepwd");
	}
...
//			}
	...
	public void datagrid(TSAgent user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        CriteriaQuery cq = new CriteriaQuery(TSAgent.class, dataGrid);
        //查询条件组装器
        ...
        cq.eq("roleType", Globals.USER_ROLE_TYPE_AGENT);
        ...
        List<TSAgent> cfeList = new ArrayList<TSAgent>();
        for (Object o : dataGrid.getResults()) {
            if (o instanceof TSAgent) {
            	TSAgent cfe = (TSAgent) o;
                if (cfe.getId() != null && !"".equals(cfe.getId())) {
                    List<TSRoleUser> roleUser = systemService.findByProperty(TSRoleUser.class, "TSAgent.id", cfe.getId());
                    if (roleUser.size() > 0) {
                        String roleName = "";
                        for (TSRoleUser ru : roleUser) {
	...
	public String deleteDialog(TSUser user,HttpServletRequest request) {
		request.setAttribute("user", user);
		return "com/rpp/agent/user-delete";
	} 
...
**/	
	...
	@ResponseBody
	public AjaxJson saveUser(HttpServletRequest req, TSAgent user) {
		String message = null;
		AjaxJson j = new AjaxJson();

		Short logType=Globals.Log_Type_UPDATE;
		// 得到用户的角色
		String roleid = oConvertUtils.getString("402881906459835f0164599a5a6e0009");
		String orgid=oConvertUtils.getString("402881906459835f016459bcbf490019");
		if (StringUtil.isNotEmpty(user.getId())) {
			TSAgent users = systemService.getEntity(TSAgent.class, user.getId());
			...
			users.setRoleType(Globals.USER_ROLE_TYPE_AGENT);
			this.userService.saveOrUpdate(users, orgid.split(","), roleid.split(","));
			message = "用户: " + users.getUserName() + "更新成功";
		} else {
			TSAgent users = systemService.findUniqueByProperty(TSAgent.class, "userName",user.getUserName());
			if (users != null) {
				...
				user.setDevFlag("1");
				user.setRoleType(Globals.USER_ROLE_TYPE_AGENT);
				//默认添加为系统用户
	...
	public ModelAndView roles(HttpServletRequest request) {
		...
		ModelAndView mv = new ModelAndView("com/rpp/agent/users");
	...
	public ModelAndView addorupdate(TSUser user, HttpServletRequest req) {
        ...
        return new ModelAndView("com/rpp/agent/user");
	}
	...
	public ModelAndView addorupdateInterfaceUser(TSUser user, HttpServletRequest req) {
		...
		req.setAttribute("tsDepart", tsDepart);
        return new ModelAndView("com/rpp/agent/interfaceUser");
	}
...
//                saveUserOrgList(req, user);
	...
	public ModelAndView userOrgSelect(HttpServletRequest request) {
		...
		return new ModelAndView("com/rpp/agent/userOrgSelect");
    }
	...
	public ModelAndView addsign(HttpServletRequest request) {
		...
		return new ModelAndView("com/rpp/agent/usersign");
	}
...
//		request.setAttribute("cssTheme", cssTheme);
		return "com/rpp/agent/changestyle";
	}
	/**
	* @Title: saveStyle
	* @Description: 修改首页样式
...
//			}
	...
	public String userSelect() {
		return "com/rpp/agent/userSelect";
	}
	...
	public ModelAndView addorupdateMyOrgUser(TSUser user, HttpServletRequest req) {
		...
		req.setAttribute("tsDepart", tsDepart);
        return new ModelAndView("com/rpp/agent/myOrgUser");
	}
}