...
//@Scope("prototype")
@Controller
@RequestMapping("/mercController")
public class MercController extends BaseController {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(MercController.class);
	...
	public String changepassword(HttpServletRequest request) {
		TSUser user = ResourceUtil.getSessionUser();
		request.setAttribute("user", user);
		return "com/rpp/merc/changepassword";
	}
	...
	public String changeportrait(HttpServletRequest request) {
		TSUser user = ResourceUtil.getSessionUser();
		request.setAttribute("user", user);
		return "com/rpp/merc/changeportrait";
	}
	...
	public String home(HttpServletRequest request) {
		TSUser user = ResourceUtil.getSessionUser();
		TSMerchant tsMerchant=systemService.findUniqueByProperty(TSMerchant.class, "id", user.getId());
		request.setAttribute("user", tsMerchant);
		return "com/rpp/merc/home";
	}
	/**
	 * 修改密码
	 *
	...
	public ModelAndView changepasswordforuser(TSUser user, HttpServletRequest req) {
		...
		return new ModelAndView("com/rpp/merc/adminchangepwd");
	}
...
//			}
	...
	public void datagrid(TSMerchant user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        CriteriaQuery cq = new CriteriaQuery(TSMerchant.class, dataGrid);
        //查询条件组装器
        ...
        cq.eq("roleType", Globals.USER_ROLE_TYPE_MERC);
	...
	public String deleteDialog(TSUser user,HttpServletRequest request) {
		request.setAttribute("user", user);
		return "com/rpp/merc/user-delete";
	} 
...
**/	
	...
	@ResponseBody
	public AjaxJson saveUser(HttpServletRequest req, TSMerchant user) {
		String message = null;
		AjaxJson j = new AjaxJson();
		TSAgent agent = systemService.findUniqueByProperty(TSAgent.class, "userName",user.getAgentCode());
		if (agent==null) {
			j.setSuccess(false);
			j.setMsg("该机构商户不存在");
			return j;
		...
		// 得到用户的角色
		String roleid = oConvertUtils.getString("402881906459835f0164599a144d0007");
		String orgid=oConvertUtils.getString("402881906459835f016459bd00c1001b");
		if (StringUtil.isNotEmpty(user.getId())) {
			TSMerchant users = systemService.getEntity(TSMerchant.class, user.getId());
			...
			users.setRoleType(Globals.USER_ROLE_TYPE_MERC);
			...
			TSMerchant users = systemService.findUniqueByProperty(TSMerchant.class, "userName",user.getUserName());
			if (users != null) {
				...
				user.setDevFlag("1");
				user.setRoleType(Globals.USER_ROLE_TYPE_MERC);
				//默认添加为系统用户
	...
	public ModelAndView roles(HttpServletRequest request) {
		...
		ModelAndView mv = new ModelAndView("com/rpp/merc/users");
	...
	public ModelAndView addorupdate(TSUser user, HttpServletRequest req) {
        ...
        return new ModelAndView("com/rpp/merc/user");
	}
	...
	public ModelAndView addorupdateInterfaceUser(TSUser user, HttpServletRequest req) {
		...
		req.setAttribute("tsDepart", tsDepart);
        return new ModelAndView("com/rpp/merc/interfaceUser");
	}
...
//                saveUserOrgList(req, user);
	...
	public ModelAndView userOrgSelect(HttpServletRequest request) {
		...
		return new ModelAndView("com/rpp/merc/userOrgSelect");
    }
	...
	public ModelAndView addsign(HttpServletRequest request) {
		...
		return new ModelAndView("com/rpp/merc/usersign");
	}
...
//		request.setAttribute("cssTheme", cssTheme);
		return "com/rpp/merc/changestyle";
	}
	/**
	* @Title: saveStyle
	* @Description: 修改首页样式
...
//			}
	...
	public String userSelect() {
		return "com/rpp/merc/userSelect";
	}
	...
	public ModelAndView addorupdateMyOrgUser(TSUser user, HttpServletRequest req) {
		...
		req.setAttribute("tsDepart", tsDepart);
        return new ModelAndView("com/rpp/merc/myOrgUser");
	}
}