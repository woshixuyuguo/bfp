	...
	<div class="form-group">
		...
		<div class="col-sm-7">
			...
			<#elseif po.showType=='checkbox' || po.showType=='radio'>
				<t:dictSelect field="${po.fieldName}" type="${po.showType?if_exists?html}" extendJson="{class:'i-checks'}" <#if po.dictTable?if_exists?html != ''><@dictInfo dictTable="${po.dictTable}" dictField="${po.dictField}" dictText="${po.dictText}" />  <#else>
	      		<@dictInfo dictField="${po.dictField}" />
			</#if> hasLabel="false"  title="${po.content}" defaultVal="${'$'}{${entityName?uncap_first}Page.${po.fieldName}}"></t:dictSelect>
			<#elseif po.showType=='select' || po.showType=='list'>
               <t:dictSelect field="${po.fieldName}" type="${po.showType?if_exists?html}" extendJson="{class:'form-control input-sm'}" <@datatype inputCheck="2" validType="${po.fieldValidType!''}" isNull="${po.isNull}" type="${po.type}" mustInput="${po.fieldMustInput!''}" isNull="${po.isNull}"/> <#if po.dictTable?if_exists?html != ''><@dictInfo dictTable="${po.dictTable}" dictField="${po.dictField}" dictText="${po.dictText}" />  <#else>
	      		<@dictInfo dictField="${po.dictField}" />
			</#if> hasLabel="false"  title="${po.content}" defaultVal="${'$'}{${entityName?uncap_first}Page.${po.fieldName}}"></t:dictSelect>
