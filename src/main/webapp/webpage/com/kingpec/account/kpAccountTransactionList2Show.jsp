...
<div class="account">
		...
		<div class="office">
				<h2>分公司账户</h2>
				<div class="office-account clearfloat">
					<div class="account-item">
						<div class="account-name">金盾石化福州北路站</div>
						<div class="account-record">
							<p class="current-record">当前金额：324,680.00 元</p>
							<p>累计充值：2,834,300.00 元</p>
							<p>累计消费：124,300.00 元</p>
						</div>
					</div>
			...
			<form id="kpAccountTransactionForm" onkeydown="if(event.keyCode==13){doSearch();return false;}">
				<div class="seerch-div">
					...
					<div class="search-control">
						<div class="type-search search-div"></div>
					</div>
				</div>
				<div class="seerch-div">
...
$(function(){
    ...
    Promise.all(promiseArr).then(function(results) {
		...
		loadSearchFormDicts($("#kpAccountTransactionForm").find(".type-search"),kpAccountTransactionListdictsData.accTran,"radio","type");
...
//获得总交易金额
function total(){
	var url="kpAccountTransactionController.do?total";
	$.ajax({
		url : url,
		type : 'post',
		data : {
		...
		success : function(data) {
			...
			if (d.success) {
				$('#in').html(d.obj.income+"<span>元</span>");
				$('#out').html(d.obj.out+"<span>元</span>");
				$('#dif').html(d.obj.dif+"<span>元</span>");
			}
		}
	});
}
...
function  branchCompany(){
	var url="kpAccountTransactionController.do?totalBranchCompany";
	$.ajax({
		url : url,
		type : 'post',
		data : {
		...
		columns:[[
			...
			,{
	            ...
	            formatter: function(value, rec, index) {
	                ...
	                var href = '';
	                if(rec.bpmStatus=='1'){
	                	href += "<a href='#'   class='ace_button'  onclick=delObj('kpAccountTransactionController.do?doDel&id=" + rec.id + "','kpAccountTransactionList')>  <i class=' fa fa-cloud-upload'></i> ";
		                href += "上传凭证</a>&nbsp;";
	                }else{
	                	href += "<a href='#'   class='ace_button'  onclick=delObj('kpAccountTransactionController.do?doDel&id=" + rec.id + "','kpAccountTransactionList')>  <i class=' fa fa-cloud-download'></i> ";
		                href += "查看凭证</a>&nbsp;";
	                }
