...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding:0px;border:0px">
		<table id="kpSiteManageList"></table>  
	</div>
	<div id = "kpSiteManageListToolbar">
		<!-- <div class="easyui-panel toolbar-search" style="display:none" data-options="doSize:false">
			<form id="kpSiteManageForm" onkeydown="if(event.keyCode==13){doSearch();return false;}">
				<div class="seerch-div">
					<label>创建日期:</label>
					<div class="search-control">
						<input type="text" name="createDate_begin" class="dts search-inp Wdate search-group-inp"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" placeholder="请选择开始时间"/>
						<span class="dts search-group-span">~</span>
						<input type="text" name="createDate_end" class="dts search-inp Wdate search-group-inp" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" placeholder="请选择结束时间"/>
					</div>
				</div>
				<div class="seerch-div">
					<label>站点名称:</label>
		...
		<div class="toolbar-btn">
			<button type="button" class="tool-btn tool-btn-default tool-btn-xs" onclick="add('录入','kpSiteManageController.do?goAdd','kpSiteManageList',768,500)">
				<i class="fa fa-plus"></i>
				<span>录入</span>
			</button>
			
			<button type="button" class="tool-btn tool-btn-default tool-btn-xs" onclick="update('编辑','kpSiteManageController.do?goUpdate','kpSiteManageList',768,500)">
				<i class="fa fa-edit"></i>
				<span>编辑</span>
			</button>
			
			<button type="button" class="tool-btn tool-btn-default tool-btn-xs" onclick="deleteALLSelect('批量删除','kpSiteManageController.do?doBatchDel','kpSiteManageList',null,null)">
				<i class="fa fa-trash"></i>
				<span>批量删除</span>
			</button>
			
			<button type="button" class="tool-btn tool-btn-default tool-btn-xs" operationCode="download" onclick="openuploadwin('Excel导入', 'kpSiteManageController.do?upload', 'kpSiteManageList')">
				<i class="fa fa-download"></i>
				<span>导入</span>
			</button>
			
			<button type="button" class="tool-btn tool-btn-default tool-btn-xs" onclick="JeecgExcelExport('kpSiteManageController.do?exportXls','kpSiteManageList')">
				<i class="fa fa-upload"></i>
				<span>导出</span>
			</button>
			
			<button type="button" class="tool-btn tool-btn-default tool-btn-xs" onclick="JeecgExcelExport('kpSiteManageController.do?exportXlsByT','kpSiteManageList')">
				<i class="fa fa-upload"></i>
				<span>模版下载</span>
			</button>
...
<script>
var kpSiteManageListdictsData = {};
$(function(){
	var promiseArr = [];
	
    Promise.all(promiseArr).then(function(results) {
    	initDatagrid();
		$('#kpSiteManageList').datagrid('getPager').pagination({
	        beforePageText: '',
	        afterPageText: '/{pages}',
...
function initDatagrid(){
	var actionUrl = "kpSiteManageController.do?datagrid&field=id,createName,createBy,createDate,updateName,updateBy,updateDate,sysOrgCode,sysCompanyCode,bpmStatus,siteName,mercNo,termNo,state,branchCompany,";
 	$('#kpSiteManageList').datagrid({
		url:actionUrl,
		idField: 'id', 
		title: '站点维护',
		loadMsg: '数据加载中...',
		...
		rownumbers:true,
		showFooter:true,
		toolbar: '#kpSiteManageListToolbar',
		frozenColumns:[[]],
		columns:[[
			...
			,{
	            ...
	            formatter: function(value, rec, index) {
	                ...
	                var href = '';
	                href += "<a href='#'   class='ace_button'  onclick=delObj('kpSiteManageController.do?doDel&id=" + rec.id + "','kpSiteManageList')>  <i class=' fa fa-trash-o'></i> ";
	                href += "删除</a>&nbsp;";
	                return href;
	            }
	        }
		...
		onLoadSuccess: function(data) {
            $("#kpSiteManageList").datagrid("clearSelections");
            if (!false) {
                if (data.total && data.rows.length == 0) {
                    var grid = $('#kpSiteManageList');
...
function reloadTable() {
	 $('#kpSiteManageList').datagrid('reload');
}
//easyui-datagrid搜索
function doSearch(){
	var queryParams = $('#kpSiteManageList').datagrid('options').queryParams;
	var actionUrl = "kpSiteManageController.do?datagrid&field=id,createName,createBy,createDate,updateName,updateBy,updateDate,sysOrgCode,sysCompanyCode,bpmStatus,siteName,mercNo,termNo,state,";
	$('#kpSiteManageForm').find(':input').each(function() {
